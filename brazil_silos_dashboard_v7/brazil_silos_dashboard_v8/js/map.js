...
const SiloMap = (() => {
  ...
  let canvasRenderer = null; // one <canvas> for all bubbles (see init)
  ...
  function makeRadiusScale(rows) {
    // Single pass instead of Math.min/max(...spread): avoids a call-stack
    // overflow on large arrays (~17k) and is measurably faster.
    let min = Infinity;
    let max = -Infinity;
    for (const r of rows) {
  ...
  function init(el, allRows) {
    ...
    // Render all bubbles onto a single <canvas> instead of one SVG node per
    // silo. With ~17k markers this collapses ~17k DOM nodes into one, keeping
    // pan/zoom and re-filtering smooth. Tooltips and popups still work.
  ...
  function render(rows) {
    ...
    rows.forEach((r) => {
      ...
      const marker = L.circleMarker([lat, lon], {
        renderer: canvasRenderer, // draw on the shared canvas, not SVG
