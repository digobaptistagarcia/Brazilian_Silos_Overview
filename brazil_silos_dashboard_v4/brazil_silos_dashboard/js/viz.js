...
const Viz = (() => {
  ...
  function renderSunburst(el, rows, valueFn, fmtValue, unit) {
    ...
    regions.forEach((reg) => {
      ...
      // inner ring — região
      arcs += `<path d="${donutPath(cx, cy, rHole, rMid, rA0, rA1)}" fill="${base}" stroke="#fff" stroke-width="1">
      ...
      reg.states.forEach((st, i) => {
        ...
        arcs += `<path d="${donutPath(cx, cy, rMid + 1, R, sA0, sA1)}" fill="${fill}" stroke="#fff" stroke-width="1">
